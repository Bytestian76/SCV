...
async function renderOrdenesList() {
    ...
    try {
        ...
        container.innerHTML = '<table><tbody>' + ordenes.map(function(item) {
            return (
                '<tr>' +
                    '<td>' +
                    ...
                    '<td>' +
                        ...
                        '<div class="cell-sub" style="max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="' + escapeHtml(item.descripcion || '') + '">' + escapeHtml((item.descripcion || '').split('\n')[0]) + '</div>' +
                    '</td>' +
                    '<td>' +
                        '<div>' + escapeHtml(item.responsable?.nombre || item.responsable_externo || 'Sin asignar') + '</div>' +
                    '</td>' +
                    '<td>' +
                        '<div class="cell-main"><span class="badge estado-' + item.estado + '">' + (LABEL_ESTADO_O[item.estado] || item.estado) + '</span></div>' +
                        '<div class="cell-sub">Prioridad: ' + (LABEL_PRIORIDAD_O[item.prioridad] || item.prioridad) + '</div>' +
                    '</td>' +
                    '<td>' +
                        '<div class="table-actions">' +
                            ...
                            '</button>' +
                            (isAdmin ? '<button class="btn-icon" title="Editar" data-action="edit" data-id="' + item.id + '">' +
                                '<svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>' +
                            '</button>' : '') +
                            (item.estado === "completada" ? '<button class="btn-icon" disabled title="No puedes cambiar el estado de una orden completada">' +
                            ...
                            '</button>' : '<button class="btn-icon" title="Cambiar Estado" data-action="cambiar-estado" data-id="' + item.id + '">' +
                                '<svg viewBox="0 0 24 24"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg>' +
                            '</button>') +
                            (isAdmin && item.estado !== "completada" ? '<button class="btn-icon btn-icon-danger" title="Eliminar" data-action="delete" data-id="' + item.id + '">' +
...
function handleOrdenesListClick(e) {
    const btn = e.target.closest('button[data-action]');
    if (!btn) return;

    const action = btn.dataset.action;
    const ordenId = parseInt(btn.dataset.id, 10);
    if (!Number.isInteger(ordenId)) return;

    handleOrdenesBtnClick(ordenId, action);
}
